...
#pragma config(Motor,  motor6,          rightDrive,    tmotorVexIQ, PIDControl, reversed, driveLeft, encoder)
...
void reOrient(float degrees, int sleepMSec){
  ...
  int reverseTurnFlag = 1;
	...
	while(abs(currentDegree - degrees) > 5.0)
	{
		...
		sleep(sleepMSec);  //wait 1 second for the robot to come to a complete stop.
